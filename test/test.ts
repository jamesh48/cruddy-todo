...
const cleanTestDatastore = () => {
  fs.readdirSync(todos.dataDir).forEach((todo: Todo) =>
...
describe("getNextUniqueId", () => {
  ...
  it("should use error first callback pattern", (done) => {
    counter.getNextUniqueId((err: any, id: string) => {
  ...
  it("should give an id as a zero padded string", (done) => {
    counter.getNextUniqueId((_err: any, id: string) => {
  ...
  it("should update the counter file with the next value", (done) => {
    ...
    counter.getNextUniqueId((_err: any, _id: null) => {
...
describe("todos", () => {
  ...
  describe("create", () => {
    ...
    it("should use the generated unique id as the filename", (done) => {
      ...
      todos.create("buy fireworks", (_err: any, _todo: Todo) => {
    ...
    it("should only save todo text contents in file", (done) => {
      ...
      todos.create(todoText, (_err: any, todo: Todo) => {
  ...
  describe("readAll", () => {
    it("should return an empty array when there are no todos", (done) => {
      todos.readAll((err: any, todoList: Todo[]) => {
        expect(err).to.be.null;
    ...
    it("should return an array with all saved todos", (done) => {
      ...
      todos.create(todo1text, (_err: any, _todo: Todo) => {
        todos.create(todo2text, (_err: any, _todo: Todo) => {
          todos.readAll((_err: any, todoList: Todo[]) => {
            expect(todoList).to.have.lengthOf(2);
            expect(todoList).to.deep.include.members(expectedTodoList, "check it out");
            done();
          });
        });
      });
  ...
  describe("readOne", () => {
    it("should return an error for non-existant todo", (done) => {
      todos.readOne("notAnId", (err: any, _todo: Todo) => {
        expect(err).to.exist;
        done();
      });
    });
    ...
    it("should find a todo by id", (done) => {
      ...
      todos.create(todoText, (_err: any, createdTodo: Todo) => {
        const id = createdTodo.id;

        todos.readOne(id, (_err: any, readTodo: Todo) => {
  ...
  describe("update", () => {
    ...
    it("should not change the counter", (done) => {
      todos.update("00001", "updated todo", (_err: any, _todo: string) => {
        const counterFileContents = fs.readFileSync(counter.counterFile).toString();
    ...
    it("should update the todo text for existing todo", (done) => {
      ...
      todos.update(todoId, updatedTodoText, (_err: any, _todo: string) => {
        const todoFileContents = fs
    ...
    it("should not create a new todo for non-existant id", (done) => {
      ...
      todos.update("00170", "bad id", (err: any, _todo: string) => {
